...
X випраити конфлікти свагера з ввходом (авторизація свагера не працює, треба входити через апі логіну вручну)

X враити баг з заходом на пост через профілб користувача (треба не `/profile/post-info/post_id` а `/post-info/post_id`)

50/50 | виправити і переробити візуал деталів поста

O | додати можливість редагування профілю
* перевірити результат API для редагування профілю (готовоно треба більше перевірок)
* додати плейсхолдери заміни для профілю (фото, біо, свої соцмережі)

50/50 | витпавити головну сторінку

X | виправити стилі пагінації (панелька переходу між сторінками)

X | виправити контейнери та їх відступи у профілі користувача

X | перегляннути пагінацію 